...
class App extends React.Component {
  ...
  async compareUsers(users) {
    ...
    async compareProfiles(profileNames) {
      const { model } = this.props;
      const results = {}; // Map<ProfileName, { objects: [], fields: [], sys: [], apex: [], vf: [] }>
      ...
      for (const name of profileNames) {
          const permSetSubQuery = `SELECT Id FROM PermissionSet WHERE PermissionSet.Profile.Name = '${name}'`;

          // Field Perms
          const fieldQuery = `SELECT Id, Field, SObjectType, PermissionsRead, PermissionsEdit FROM FieldPermissions WHERE ParentId IN (${permSetSubQuery})`;
          const fieldRes = await model.spinFor(sfConn.rest("/services/data/v" + apiVersion + "/query/?q=" + encodeURIComponent(fieldQuery)));
          
          // Object Perms
          const objQuery = `SELECT Id, ParentId, SObjectType, PermissionsRead, PermissionsCreate, PermissionsEdit, PermissionsDelete, PermissionsViewAllRecords, PermissionsModifyAllRecords FROM ObjectPermissions WHERE ParentId IN (${permSetSubQuery})`;
          ...
          // Apex Class Access
          const apexQuery = `SELECT Name FROM ApexClass WHERE Id IN (SELECT SetupEntityId FROM SetupEntityAccess WHERE SetupEntityType = 'ApexClass' AND ParentId IN (${permSetSubQuery}))`;
          const apexRes = await model.spinFor(sfConn.rest("/services/data/v" + apiVersion + "/query/?q=" + encodeURIComponent(apexQuery)));

          // VF Page Access
          const vfQuery = `SELECT Name FROM ApexPage WHERE Id IN (SELECT SetupEntityId FROM SetupEntityAccess WHERE SetupEntityType = 'ApexPage' AND ParentId IN (${permSetSubQuery}))`;
          const vfRes = await model.spinFor(sfConn.rest("/services/data/v" + apiVersion + "/query/?q=" + encodeURIComponent(vfQuery)));
          ...
          if (apexRes && apexRes.records) {
              apexRes.records.forEach(r => {
                  profileData.apex[r.Name] = "true";
              });
          }

          if (vfRes && vfRes.records) {
              vfRes.records.forEach(r => {
                  profileData.vf[r.Name] = "true";
              });
          }
      ...
      Object.values(results).forEach(d => {
          ...
          Object.keys(d.apex).forEach(k => allApex.add(k));
          Object.keys(d.vf).forEach(k => allVf.add(k));
      });

      const sortedObjects = Array.from(allObjects).sort();
      const sortedFields = Array.from(allFields).sort();
      const sortedSys = Array.from(allSys).sort();
      const sortedApex = Array.from(allApex).sort();
      const sortedVf = Array.from(allVf).sort();
              ...
              sections: [
                 { 
                  ...
                  { 
                      ...
                      keys: sortedApex, 
                      subCols: ["Access"],
                      getData: (itemName, key) => ({ Access: results[itemName]?.apex[key] })
                  },
                  { 
  ...
  processProfileComparison(itemNames, allMetadata) {
    ...
    if (comparisonData.type === "structured_profile_transposed") {
        ...
        return h("div", { className: "slds-card slds-m-top_medium", style: { flex: 1, display: "flex", flexDirection: "column", overflow: "hidden", minHeight: "200px" } },
            // Tab Bar
            h("div", { className: "slds-card__header slds-grid slds-p-bottom_none", style: { overflowX: "auto", flexShrink: 0, borderBottom: "1px solid #dddbda" } },
            ...
            // Filter & Action Bar
            h("div", { className: "slds-p-around_small slds-border_bottom", style: { display: "flex", alignItems: "center", justifyContent: "space-between", backgroundColor: "#f3f2f2" } },
               h("div", { className: "slds-form-element", style: { maxWidth: "300px", width: "100%" } },
               ...
               h("div", { className: "slds-button-group" },
                   h("button", { className: "slds-button slds-button_neutral" }, "Profile Filter"),
                   h("button", { className: "slds-button slds-button_success" }, "Export XLSX"),
                   h("button", { className: "slds-button slds-button_success" }, "Export CSV"),
                   h("button", { className: "slds-button slds-button_icon-border-filled", title: "Refresh" }, "↻")
               )
            ),
