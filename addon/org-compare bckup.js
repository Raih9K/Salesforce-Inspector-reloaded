...
const PROFILE_SECTIONS = [
  "System Permissions", "Object Permissions", "Field-Level Security",
  "Page Layout Assignment", "Visualforce Page Access", "Tab Settings",
  "App Settings", "Login Restriction"
];

const PERM_SET_SECTIONS = [
  "System Permissions", "Object Permissions", "Field-Level Security",
  "Apex Class Access", "Visualforce Page Access", "Tab Settings",
  "App Settings", "Permission Set Group", "Permission Set Assignment"
];
...
class App extends React.Component {
  ...
  async compareUsers(users) {
    ...
    async compareProfiles(profileItems) {
      const { model } = this.props;
      const { activeSubSections } = this.state;
      ...
      // 1. Dynamic System Permissions Discovery (Needed if System Permissions selected)
      let sysPermFields = [];
      if (activeSubSections.includes("System Permissions")) {
          const permSetDescribe = await model.spinFor(sfConn.rest("/services/data/v" + apiVersion + "/sobjects/PermissionSet/describe"));
          sysPermFields = permSetDescribe.fields
      ...
      // Check if ANY of these are needed to avoid redundant fetches if possible, though Query handles filtering.
      // But query is specifically for SetupEntityAccess.
      const neededSetupTypes = [];
      if (activeSubSections.includes("Apex Class Access")) neededSetupTypes.push("'ApexClass'");
      if (activeSubSections.includes("Visualforce Page Access")) neededSetupTypes.push("'ApexPage'");
      ...
      if (activeSubSections.includes("Page Layout Assignment")) {
      ...
      if (activeSubSections.includes("Login Restriction")) {
          try {
              // 1. IP Ranges
      ...
      const filteredSections = sections.filter(s => activeSubSections.includes(s.title));
  ...
  async comparePermSets(permSetItems) {
      ...
      const psNames = permSetItems.map(p => p.Label || p.Name); 
      const psMap = {};
      permSetItems.forEach(p => psMap[p.Id] = p.Label || p.Name);

      // Pass active sections specifically to fetcher? 
      // Current fetchPermissionsForPermSets fetches everything. Let's optimize it.
      const rawData = await this.fetchPermissionsForPermSets(psIds, activeSubSections);
      
      const results = {};
      
      // Aux Data: Groups
      const psGroups = {};
      if (activeSubSections.includes("Permission Set Group")) {
          const groupQuery = `SELECT PermissionSetId, PermissionSetGroup.MasterLabel FROM PermissionSetGroupComponent WHERE PermissionSetId IN (${psIds.map(id => `'${id}'`).join(",")})`;
          const groupRes = await model.spinFor(sfConn.rest("/services/data/v" + apiVersion + "/query/?q=" + encodeURIComponent(groupQuery)));
          if (groupRes && groupRes.records) {
              groupRes.records.forEach(r => {
                  if (!psGroups[r.PermissionSetId]) psGroups[r.PermissionSetId] = [];
                  psGroups[r.PermissionSetId].push(r.PermissionSetGroup?.MasterLabel || "Unknown Group");
              });
          }
      }

      // Aux Data: Assignments
      const psUsers = {};
      if (activeSubSections.includes("Permission Set Assignment")) {
          const assignQuery = `SELECT PermissionSetId, Assignee.Name FROM PermissionSetAssignment WHERE PermissionSetId IN (${psIds.map(id => `'${id}'`).join(",")})`;
          const assignRes = await model.spinFor(sfConn.rest("/services/data/v" + apiVersion + "/query/?q=" + encodeURIComponent(assignQuery)));
          if (assignRes && assignRes.records) {
              assignRes.records.forEach(r => {
                  if (!psUsers[r.PermissionSetId]) psUsers[r.PermissionSetId] = [];
                  if (r.Assignee) psUsers[r.PermissionSetId].push(r.Assignee.Name);
              });
          }
      }
      ...
      psIds.forEach(id => {
          results[psMap[id]] = {
              ...rawData[id],
              groups: psGroups[id] || [],
              assignments: psUsers[id] || []
          };
      });

      // Build sorting keys for new sections
      const createSortedKeys = (category) => {
          const s = new Set();
          Object.values(results).forEach(d => {
             if (category === "groups") d.groups.forEach(k => s.add(k));
             if (category === "assignments") d.assignments.forEach(k => s.add(k));
          });
          return Array.from(s).sort();
      };

      const sortedGroups = createSortedKeys("groups");
      const sortedAssignments = createSortedKeys("assignments");

      // Helper to generate keys/sections from sub-methods
      const baseData = this.buildComparisonDataResult(psNames, results); 
      // Base data gives us the standard sections. We need to Reorder and Add new ones.

      // Map base sections to a lookup
      const sectionMap = {};
      baseData.sections.forEach(s => sectionMap[s.title] = s);
      
      // Custom Sections
      const newSections = [
          // 1. System
          sectionMap["System Permissions"],
          // 2. Object
          sectionMap["Object Permissions"],
          // 3. FLS
          { ...sectionMap["Field Permissions"], title: "Field-Level Security" },
          // 4. Apex
          sectionMap["Apex Class Access"],
          // 5. VF
          sectionMap["Visualforce Page Access"],
          // 6. Tab
          sectionMap["Tab Settings"],
          // 7. App
          { ...sectionMap["App Visibility"], title: "App Settings" },
          // 8. Perm Set Group
          {
              title: "Permission Set Group",
              keys: sortedGroups,
              subCols: ["Member"],
              getData: (itemName, key) => ({ Member: results[itemName]?.groups.includes(key) })
          },
          // 9. Assignment
          {
               title: "Permission Set Assignment",
               keys: sortedAssignments,
               subCols: ["Assigned"],
               getData: (itemName, key) => ({ Assigned: results[itemName]?.assignments.includes(key) })
          }
      ];
      ...
      this.setState({
          comparisonData: {
              ...baseData,
              sections: newSections.filter(s => s && activeSubSections.includes(s.title)) 
          }
      });
  }
  ...
  async fetchPermissionsForPermSets(psIds, activeSections) {
      ...
      // Default to checking all if activeSections not provided (backward compat)
      const isIncluded = (sec) => !activeSections || activeSections.includes(sec);

      // Parallel Fetch
      const promises = [];

      // A. Objects
      if (isIncluded("Object Permissions")) {
      ...
      // B. Fields
      if (isIncluded("Field-Level Security")) {
      ...
      if (isIncluded("System Permissions")) {
      ...
      const neededSetupTypes = [];
      if (isIncluded("Apex Class Access")) neededSetupTypes.push("'ApexClass'");
      if (isIncluded("Visualforce Page Access")) neededSetupTypes.push("'ApexPage'");
      if (isIncluded("App Settings")) neededSetupTypes.push("'TabSet'");

      if (neededSetupTypes.length > 0) {
          promises.push(sfConn.rest("/services/data/v" + apiVersion + "/query/?q=" + encodeURIComponent(`SELECT ParentId, SetupEntityId, SetupEntityType, SetupEntity.Name FROM SetupEntityAccess WHERE ParentId IN (${idsString}) AND SetupEntityType IN (${neededSetupTypes.join(",")})`))
            .then(res => res.records.forEach(r => {
  ...
  processProfileComparison(itemNames, allMetadata) {
    ...
    if (comparisonData.type === "structured_profile_transposed") {
        ...
        // Filter Logic (Lifted up to calculate count)
        const allKeys = activeSection.keys || [];
        const filteredKeys = allKeys.filter(k => k.toLowerCase().includes((tableFilter || "").toLowerCase()));
                   ...
                   (selectedCategory.id === "profiles" ? PROFILE_SECTIONS : PERM_SET_SECTIONS).map(sec => {
                       ...
                       return h("button", {
                           ...
                           style: {
                               ...
                               color: isActive ? "white" : "#080707",
                               border: `1px solid ${isActive ? "#008a90" : "#dddbda"}`,
                               borderRadius: "4px",
                               padding: "4px 12px",
                               fontSize: "0.80rem",
                               lineHeight: "1.2",
